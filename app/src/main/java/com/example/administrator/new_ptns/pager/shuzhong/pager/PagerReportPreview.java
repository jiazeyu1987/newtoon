...
import com.example.administrator.new_ptns.PlayMediaActivity;
...
public class PagerReportPreview {
    ...
    public ConstraintLayout media_holder;
    CustomItem5 tag1;
    ...
    public void init_view(View view){
        ...
        list1.add(new OperationTempData(1,1,"2020-2-2","STN","3+ 2-","200","xxxxx"));
        list1.add(new OperationTempData(1,1,"2020-2-2","STN","3+ 2-","200","xxxxx"));
        list1.add(new OperationTempData(1,1,"2020-2-2","STN","3+ 2-","200","xxxxx"));
        list1.add(new OperationTempData(1,1,"2020-2-2","STN","3+ 2-","200","xxxxx"));
        list1.add(new OperationTempData(1,2,"2020-2-2","STN","3+ 2-","200","xxxxx"));
        list1.add(new OperationTempData(1,1,"2020-2-2","STN","3+ 2-","200","xxxxx"));
        list1.add(new OperationTempData(1,1,"2020-2-2","STN","3+ 2-","200","xxxxx"));
        list1.add(new OperationTempData(1,1,"2020-2-2","STN","3+ 2-","200","xxxxx"));
        list1.add(new OperationTempData(1,1,"2020-2-2","STN","3+ 2-","200","xxxxx"));
        ...
        media_holder = mView.findViewById(R.id.sdfs);
        set_data();
        set_table_data();
    }
    ...
    private void set_data(){
        ...
        pref = mContext.getSharedPreferences(BLEManager.BLE_PREF, mContext.MODE_PRIVATE);
        editor = pref.edit();

        ArrayList<String> picturelist = G.g_patient_data.latest_patientOperationData.operation_picture_path;
        for (int i = 0; i < picturelist.size(); i++) {
            String path1 = picturelist.get(i);
            ImageView imageView = new ImageView(mContext);
            ConstraintLayout.LayoutParams paras = new ConstraintLayout.LayoutParams(512,512);
            paras.leftToLeft=R.id.sdfs;
            paras.topToTop = R.id.sdfs;
            paras.leftMargin=532*(i%3);
            paras.topMargin =532*(i/3);
            imageView.setLayoutParams(paras);
            media_holder.addView(imageView); //动态添加图片
            Album.getAlbumConfig()
                    .getAlbumLoader()
                    .load(imageView, path1);
