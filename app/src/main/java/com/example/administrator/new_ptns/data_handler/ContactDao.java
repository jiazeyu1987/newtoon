...
public class ContactDao {
    ...
    private String db_name = "contact2.db";
    ...
    public void insert(ContactData contact1){
        ...
        db.execSQL("insert into t_contact(id_card,date_1) values(?,?)" , new Object[]{contact1.id_card,date1});
        db.close();
    }
    ...
    public void modify(ContactData contact1){
        ...
        Gson gson = new Gson();
        String date1 = gson.toJson(contact1);
        db.execSQL("update t_contact set id_card=?,date_1=? where id=?" , new Object[]{date1,contact1.id_card, contact1.id});
        db.close();
    }

    public ArrayList<ContactData> get_contact_by_idcard(String id_card1){
        ArrayList<ContactData> list = new ArrayList<ContactData>();
        helper = new ContactHelper(context,db_name, null, 1);
        SQLiteDatabase db = helper.getWritableDatabase();
        Cursor cursor = db.rawQuery("select id,date_1 from t_contact where id_card=?", new String[]{id_card1});
        if(cursor == null){
            return null;
        }
